# Strata Forge: AI Assistant Instructions

This document provides instructions for AI models (ChatGPT, Claude, etc.) to help developers work with **Strata Forge** schemas.

## 🏗️ The "@strata" Standard

Strata Forge uses JSDoc comments to store visual metadata directly in the `schema.ts` file. This avoids sidecar JSON files and keeps the code as the single source of truth.
...
*Note: Always ensure that `relations()` identifiers follow the naming convention `${tableName}Relations` for automatic parsing.*
