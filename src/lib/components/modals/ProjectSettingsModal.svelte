...
<script lang="ts">
  ...
  let preferredEditor = $state(
    (typeof localStorage !== "undefined" && localStorage.getItem("strata_preferred_editor")) || "auto"
  );
  let isDetecting = $state(false);

  $effect(() => {
    if (typeof localStorage !== "undefined" && preferredEditor) {
      localStorage.setItem("strata_preferred_editor", preferredEditor);
    }
  });
  ...
  async function createNewWranglerFile() {
    if (schemaState.isSandboxMode || !schemaState.filePath) return;
    const pathToCheck = wranglerPath.trim() || "wrangler.jsonc";
    const absPath = resolvePath(schemaState.filePath, pathToCheck);
    const content = (absPath.endsWith(".json") || absPath.endsWith(".jsonc"))
      ? `{\n  "$schema": "node_modules/wrangler/config-schema.json",\n  "name": "my-worker",\n  "main": "src/index.ts",\n  "compatibility_date": "2024-01-01"\n}\n`
      : `# Cloudflare Worker Configuration\nname = "my-worker"\nmain = "src/index.ts"\ncompatibility_date = "2024-01-01"\n`;
    try {
      await PlatformService.writeText(absPath, content);
      toast.success("Wrangler Configuration Created", {
        description: `Created file at ${pathToCheck}`,
      });
      wranglerPath = pathToCheck;
    } catch (e: any) {
  ...
  async function handleSave(e: Event) {
    ...
    try {
      ...
      toast.success("Settings Saved", {
        description: "Project configuration updated in @strata-layout manifest.",
      });
    } catch (err: any) {
...
<div
  ...
  onclick={(e) => e.target === e.currentTarget && (schemaState.showProjectSettingsModal = false)}
>
  <div
  ...
  >
    ...
    <form onsubmit={handleSave} class="p-6 flex flex-col gap-6">
                ...
                {:else if validationStatus === "invalid"}
                  ...
                  <div class="flex flex-col gap-1.5">
                    ...
                    {#if !schemaState.isSandboxMode}
                      <button
                        type="button"
                        class="btn btn-xs btn-outline btn-error font-bold rounded-field self-start mt-1"
                        onclick={createNewWranglerFile}
                      >
      ...
      <div class="flex flex-col gap-2 pt-2 border-t border-base-300/60">
        <label
          for="editor-preference-select"
          class="text-[10.5px] font-bold uppercase tracking-wider text-base-content/80 block"
        >
          Preferred Code Editor
        </label>
        <select
          id="editor-preference-select"
          bind:value={preferredEditor}
          class="select select-bordered select-sm w-full rounded-field font-sans text-xs bg-base-100"
        >
          <option value="auto">Auto-Detect (VS Code / Cursor)</option>
          ...
          <option value="system">System Default Application</option>
        </select>
        <span class="text-[10px] text-base-content/60 leading-normal">
          Clicking table files or double-clicking nodes will jump straight to the declaration line in this editor.
        </span>
      </div>
