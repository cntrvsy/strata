...
<script lang="ts">
  ...
  const form = superForm(defaults(valibot(tableSchema)), {
    ...
    async onUpdate({ form }) {
      if (form.valid && schemaState.filePath) {
        ...
        const extra = form.data.target === "do" ? {
          class: doClass.trim() || undefined,
          path: doPath.trim() || undefined
        } : undefined;

        await schemaState.addTable(form.data.name, form.data.target as any, extra);
  ...
  const targetConfig = {
    d1: { label: "D1 Database Table", icon: Database, color: "text-primary", details: "Creates a standard sqliteTable code structure." },
    do: { label: "Durable Object Pointer", icon: Cpu, color: "text-secondary", details: "Registers a class method signature binding." },
    kv: { label: "KV Namespace Pointer", icon: Zap, color: "text-accent", details: "Creates an inline or JSDoc-annotated schema entry." },
    r2: { label: "R2 Bucket Pointer", icon: HardDrive, color: "text-info", details: "Creates an empty object mapping directory hierarchies." }
  };
</script>
...
>
  ...
  >
    ...
    <form use:enhance class="p-8 flex flex-col gap-5">
      <!-- Entity Name -->
        ...
        <Form.Control>
          {#snippet children({ props })}
            ...
            <select
              ...
              class="select select-bordered w-full rounded-lg bg-base-200/50 border-base-300 focus:border-primary transition-all text-sm"
            >
      ...
      {#if $formData.target === "do"}
        <div class="p-4 bg-secondary/5 border border-secondary/10 rounded-2xl flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-200">
          <div class="flex flex-col gap-1.5">
            <label for="do-class-input" class="text-[10px] font-bold opacity-60 uppercase">Target Class Name</label>
            <input
              id="do-class-input"
              bind:value={doClass}
              placeholder="e.g. UserSession"
              class="input input-sm input-bordered w-full rounded-lg bg-base-100/50 border-base-300 focus:border-secondary transition-all font-mono text-xs"
              required
            />
          </div>
          <div class="flex flex-col gap-1.5">
            <label for="do-path-input" class="text-[10px] font-bold opacity-60 uppercase">Target TS File Path</label>
            <input
              id="do-path-input"
              bind:value={doPath}
      ...
      <div class="p-4 bg-info/5 border border-info/10 rounded-2xl text-[11px] leading-relaxed flex gap-2">
        ...
        <div class="flex flex-col gap-1 text-base-content/85">
          <span class="font-bold text-base-content">
            {targetConfig[($formData.target as keyof typeof targetConfig) || "d1"].label}
          </span>
          <span>
            {targetConfig[($formData.target as keyof typeof targetConfig) || "d1"].details}
          </span>
        </div>
      </div>
      ...
      <div class="mt-4 flex flex-col gap-3">
        <button
          type="submit"
          class="btn btn-primary btn-lg rounded-lg w-full shadow-xl shadow-primary/20 font-bold"
        >
          Forge Entity
        </button>
        <p class="text-[10px] text-center opacity-40 leading-relaxed px-4">
          This will inject a new entity block into your <code>schema.ts</code> and sync immediately.
        </p>
      </div>
    </form>
  </div>
