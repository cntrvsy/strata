    ...
    >
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2 grow">
          {#if editingColumnName === col.name}
            <div class="flex items-center gap-1 grow">
              <input
                ...
                class="input input-xs input-bordered w-full rounded-lg font-bold text-[13px] h-7 bg-base-100 focus:border-primary transition-all font-mono"
          ...
          {:else}
            <div class="flex items-center gap-2 group/col-title">
              <span class="font-mono text-xs font-bold group-hover/field:text-primary transition-colors">
                {col.name}
              </span>
              {#if !isReadOnly}
                <button
        ...
        <div class="flex items-center gap-2">
          <span class="text-[9px] font-mono opacity-50 uppercase bg-info/10 text-info px-1.5 py-0.5 rounded leading-none shrink-0 font-bold">
            {col.definition}
          </span>
          {#if !isReadOnly}
            <button
