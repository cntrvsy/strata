...
>
  {#if isCollapsed}
    <button
      ...
      class="btn btn-sm bg-base-100/90 backdrop-blur-md border border-base-300 rounded-full shadow-lg hover:border-primary/40 flex items-center gap-2 px-4 py-2 text-xs font-bold transition-all"
    >
      <Layers class="w-3.5 h-3.5 text-primary" />
      <span class="opacity-60 text-[10px] uppercase tracking-wider">Stats</span>
      <span class="badge badge-primary badge-xs scale-90">{stats.total}</span>
    </button>
  {:else}
    <div
      class="bg-base-100/90 backdrop-blur-md border border-base-300 rounded-2xl shadow-xl p-4 min-w-[200px]"
      transition:slide={{ duration: 200 }}
    >
      <div class="flex items-center justify-between gap-4 mb-4 border-b border-base-300 pb-2">
      ...
      <div class="flex flex-col gap-3">
        ...
        <!-- General Stats -->
        <div class="flex flex-col gap-1.5 px-1 border-t border-base-200 pt-2.5">
          <div class="flex items-center justify-between text-[10px]">
            <span class="opacity-40 font-medium">Total Entities</span>
            <span class="font-mono font-bold">{stats.total}</span>
          </div>
          <div class="flex items-center justify-between text-[10px]">
            <span class="opacity-40 font-medium">Total Fields</span>
            <span class="font-mono font-bold">{stats.columns}</span>
          </div>
          <div class="flex items-center justify-between text-[10px]">
            <span class="opacity-40 font-medium">Active Edges</span>
            <span class="font-mono font-bold text-primary">{stats.relations}</span>
          </div>
          {#if schemaState.activeCoordinates}
            <div class="flex items-center justify-between text-[10px]">
              <span class="opacity-40 font-medium">Coordinates</span>
