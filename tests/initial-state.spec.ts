...
test.describe('Initial State & Navigation', () => {
  ...
  test('shows empty state when no file is loaded', async ({ page }) => {
    // 1. Check for the Welcome to Strata heading in the overlay
    await expect(page.getByRole('heading', { name: /welcome to strata/i })).toBeVisible({ timeout: 15000 });
    await expect(page.getByText('Open a local Drizzle')).toBeVisible();
  ...
  test('navbar reflects empty state', async ({ page }) => {
    await expect(page.getByTestId('titlebar').getByText('Open Schema...')).toBeVisible();
  });
});
